...
struct packetData *PacketStored;
...
void A_output(struct msg message)
{
        ...
        PacketStored[NextPosition].Acked = false;
        PacketStored[NextPosition].Sent = false;
 ...
 *   *   *   *   *                              {*/
                        ...
                        if (PacketStored[NextSeqNum].Sent == false && PacketStored[NextSeqNum].Acked == false)
                        {
 ...
 *   *   *   *                                  */
                                ...
                                PacketStored[NextSeqNum].Sent = true;
...
{       /*set isSet to true*/
        ...
        {       Ack_received_at=get_sim_time();
                ...
                {
                        PacketStored[i].Sent=true;
                        PacketStored[i].Acked=false;
                }
  ...
  /*printf("AI: NextSeqNum is %d and NextPosition is %d and LastPacketSent is %d. Sending buffered packets \n", NextSeqNum, NextPosition, LastPacketSent);*/
                ...
                {
                        if(PacketStored[i].Sent==false && PacketStored[i].Acked==false)
                        {
...
void A_timerinterrupt()
{
        ...
        for(i=base;i<=NextSeqNum && PacketStored[i].Sent==true && PacketStored[i].Acked==false;i++)
        {
...
void A_init()
{
       ...
       {TIMEOUT=11.5; /* was 30, 25*/
        ...
        PacketStored=(struct packetData*) malloc(BUFSIZE*sizeof(struct packetData));
        /*TIMEOUT=5.0;*/
}
